...
>   - [安装wiringPi](http://wiringpi.com/wiringpi-updated-to-2-52-for-the-raspberry-pi-4b/)：aarch64需要 git 下载编译安装
...
> - http://archive.raspberrypi.org/debian/pool/main/r/raspberrypi-firmware/：树莓派旧文件归档。可以通过 `sudo dpkg -i raspberrypi-kernel_*.deb` 的方式回滚内核。
> - https://downloads.raspberrypi.org/：这里可以下载各个版本的树莓派镜像
> - [The Linux kernel](https://www.raspberrypi.com/documentation/computers/linux_kernel.html)：这里有教如何编译一个 linux kernel，然后最新版本的 head 可以通过 `sudo apt install linux-headers-rpi-{v6,v7,v7l}` 这样的方式下载，详细看文档。
...
- 开启`SSH`：`sudo raspi-config`；也可以在 boot 目录下直接创建ssh文件
...
- 可以在 `/boot/config.txt`  中指定 `kernel=kernel8-custom.img` 内核，这样可以把默认的 v8 内核切换到有 `header` 的 `v7l+` 内核
